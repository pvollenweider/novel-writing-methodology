...
- **Tension** — is there an active question pulling the reader forward at every point, or a stretch where nothing is genuinely uncertain? For a passage where the argument itself is the engine (see Constitution item 2), first identify which shape the demonstration is actually using — sequential (order load-bearing), cumulative (saturation/repetition load-bearing, order much less so), or formal (the reader's recognition of a pattern developing, independent of which particular instances fill it) — then apply the destructive test built for that shape, not a default one: permute major units for a sequential demonstration; remove a substantial fraction of units for a cumulative one (a catalog can fail a permutation test and still be doing real work — thinning, not reordering, is the test that would actually break it); break or reorder the pattern itself for a formal one. The general question underneath all three: could this material's experience be replaced by a concise statement of its conclusion without losing something essential? If yes, it's lecturing regardless of surface properties; if no, it's moving, whichever shape is doing the work.
