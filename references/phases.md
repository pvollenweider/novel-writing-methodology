...
- **Tension** — is there an active question pulling the reader forward at every point, or a stretch where nothing is genuinely uncertain? For a passage where the argument itself is the engine (see Constitution item 2), test it on the argument's own terms: does this turn foreclose what could have been believed before it (irreversibility), does the stakes/scope/cost of the position increase (escalation) rather than just accumulate, is there a consciousness with something real to lose in how it lands, and is it carried by concrete particulars rather than a paraphrasable claim? A demonstration passing all four is functioning as real causality; one passing none of them is a lecture with dialogue tags, independent of how well-written the prose is.
