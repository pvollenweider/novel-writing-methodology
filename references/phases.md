...
**Do:** grade sources by reliability tier (primary/official, secondary-quality, unofficial compilation) and never let a low-tier source carry a claim that needs a high-tier one. Pre-register what a piece of research is being used to test *before* reading it, where practical — this limits the temptation to read sources into confirming what's already assumed. Document access limits (a blocked source, an unverifiable claim) as findings, not as gaps to quietly paper over later. When the author supplies a personal primary source directly (a GPS track of a real hike, a personal document, a firsthand account) rather than something researched, treat it as the highest tier without independent re-verification — it outranks a contradicting web search by default, precisely because it's the one kind of source Phase 1's usual tiering can't rate higher than.

**On research whose subject is itself capable of harm** (a fraud mechanism, a method of violence, a manipulation technique): the "known-but-not-shown" bucket above needs a sharper edge here than for ordinary research. Split findings into what characterizes the *human* side (psychology, warning signs, what it looks and feels like from inside or nearby — usable, often necessary) and what would function as a reproducible *procedure* if extracted from the fiction (operational steps a reader could follow) — the latter belongs in the never-shown bucket categorically, not case-by-case. When research produces a precise technical fact (an exact material property, a verified numeric spec, a professional term of art), the general default is to let only the *phenomenological effect* of that fact reach the prose — what a character felt, noticed, or experienced because it was true — and keep the technical designation itself in the research notes; this applies most visibly to specialized/technical material (gear, procedure, jargon) but the same discipline is what keeps a reproducible procedure out of the text in the harm-capable case above.
...
**A structural element built specifically for its surprise value — a twist, a reveal, a bifurcation — should be generated only once its intended carriers (the characters or world meant to deliver it) already exist**, not before. Inventing the surprise first and then building or bending a character to deliver it tends to produce an element optimized for landing as a surprise rather than one that follows from a character the reader already believes in; the order matters enough to be worth stating as its own step, separate from the general generation-before-selection discipline above. Log the order in the exploration candidates register (see `registers.md`) when it's a genuine risk.
...
**A sharper version of the "atmosphere" answer, for a scene that carries no plot function at all:** the scene earns its place if *its own existence, independent of the plot,* becomes interesting enough to make the reader temporarily stop asking what happens next — not merely "nothing happens here, but it's pleasant" or "it sets a mood." The test is whether the scene actively suspends the plot-question rather than just failing to advance it; a scene that does neither (doesn't advance the plot and doesn't earn the reader's attention on its own terms) is a real candidate for cutting even though it "provides atmosphere" in the loosest sense of that phrase.
...
**A formal or structural device meant to demonstrate its own limits (see Constitution item 2) is stronger if it fails from inside the story than if the author repairs it from outside.** If a character relies on some explicit system — a rule, a category, a method — that the book means to show as insufficient, let the insufficiency show up as the character's own lived experience of the system not holding (they reach for it and it doesn't fit, in front of the reader) rather than as the author quietly adding a new category, rule, or exception that patches the gap. The first is the demonstration doing its own work; the second explains the demonstration's limit instead of dramatizing it.

**A line that carries the book's central motif — especially one tied to its title or opening — should not be paraphrased with variation at its dramatic payoff.** If the text is going to pay off that line later, let the character's change register in what they *do* differently, not in an echoed or reworded version of the line itself; a variation on the line at the climax tends to read as the author underlining the connection rather than the reader discovering it.
...
**If this book is part of a multi-book cycle and a character here appeared, even peripherally, in an already-frozen book:** run the cycle-continuity protocol (`protocols.md`) before or alongside this phase's seven questions, not after — building the character here should still happen on their own terms first, but the frozen occurrences need to be enumerated early enough that this phase's answers don't contradict them by accident.
...
**The blind boundary pass, for converting draft units into final chapter breaks.** Once a manuscript exists in smaller working units (scenes, movements, lots) than its intended final chapters, test every candidate boundary between them individually and mechanically: *what does the reader lose if this specific boundary is removed and the two units flow together instead?* Run this on all of them before deciding on a final chapter count — do not set a target chapter count in advance and then look for that many boundaries to keep; let the boundaries that survive the test determine the count. Expect an asymmetric result (one very long chapter next to several short ones); that asymmetry is a legitimate outcome of the test, not a defect to smooth away by force-splitting or force-merging afterward, if it's consistent with what the book is otherwise doing (an accumulating stretch followed by a contraction, say). Two adjacent units merged this way can also fail their own separate montage test even after passing the boundary-removal test — see the montage/juxtaposition test in `protocols.md`, which checks proximity effects the boundary test doesn't.
...
**Once a scene is drafted, the Phase 2/3 documents that planned it stop being the authority on what the scene does.** If a function assigned to a scene by the architecture map or character register isn't actually perceptible in the drafted text, the planning document doesn't get to override that — the diagnosis runs on what a reader can actually find on the page, not on what was intended to be there. Treat every audit below as reading the text, not the plan.
...
- **Tension** — is there an active question pulling the reader forward at every point, or a stretch where nothing is genuinely uncertain? For a passage where the argument itself is the engine (see Constitution item 2), first identify which shape the demonstration is actually using — sequential (order load-bearing), cumulative (saturation/repetition load-bearing, order much less so), or formal (the reader's recognition of a pattern developing, independent of which particular instances fill it) — then apply the destructive test built for that shape, not a default one: permute major units for a sequential demonstration; remove a substantial fraction of units for a cumulative one (a catalog can fail a permutation test and still be doing real work — thinning, not reordering, is the test that would actually break it); break or reorder the pattern itself for a formal one. The general question underneath all three has two stages: first, could this material's experience be replaced by a concise statement of its conclusion without losing something essential? If yes, the passage is probably substitutable by exposition, regardless of surface properties. If no, identify what the irreducible experience actually changes across the passage — non-reducibility alone (voice, texture, sentence-level pleasure) does not establish directional movement; a set of individually vivid, interchangeable instances of the same point can resist summary and still not be going anywhere.
...
**Do:** where a finding comes from a statistical inventory (e.g. counting a recurring device across the manuscript), **re-test it functionally, occurrence by occurrence**, before treating it as a homogeneous defect. A pattern detected by raw count frequently turns out to be two distinct families that only looked like one thing in aggregate, or to be doing different, necessary work in each instance. A practical grid for this: sort each occurrence into **keep** (functional, no action), **ignore** (neutral, not worth the cost of touching), **candidate** (worth discussing, genuinely suspect), or **out of scope** (already validated by an earlier pass) — and note that the bar for what counts as *candidate* should rise, not stay fixed, as a manuscript goes through more passes and stabilizes; the same repetition that was worth flagging on an early, rougher draft may be exactly the kind of thing a later, more polished draft can least afford to keep second-guessing without a real reason.

**Two after-the-fact checks worth running once a correction from this phase has actually been applied, not only before:**
- **Did the same fix, applied to several occurrences of one defect, create a new pattern of its own?** The same solution repeated at every occurrence of a problem can produce a new, detectable over-regularity — the fix recreating the defect it was meant to remove, in a different shape. Re-run the relevant audit after applying a repeated fix, not only before.
- **Once a specific tic or defect has been named and corrected in some instances, check that the correction isn't being applied by category rather than by instance.** A named defect makes every superficially similar-looking passage look suspect; verify each remaining occurrence on its own merits (does *this one* actually do the thing that was diagnosed as wrong?) rather than extending the correction to anything that resembles the flagged pattern.
...
**Do:** mark claims that couldn't be verified (blocked source, no primary document available) as explicitly unresolved rather than silently accepting or silently cutting them — an honest "unverified" is more useful later than a guess dressed as a check. When a real-world detail can't be pinned down with confidence (an exact place name, a specific date, a precise figure), default to the more generic true statement rather than inventing a specific false one — a vaguer but unverifiably-wrong detail is a smaller failure than a precise, checkable one that turns out to be fabricated.

**For a licensed reconstruction built from an identifiable real source** (see Constitution item 7): transpose the *function* the real detail serves, not its identifying context. If a real person's documented pattern of behavior is being fictionalized, keep what that pattern *does* structurally (what it signals, what it costs, what it enables) and change the specific, identifying particulars that would let a reader trace the fiction back to the real source. Check the swap by asking whether the generic version still produces the same effect on the reader without making the source recognizable — if it does, the transposition succeeded; if the effect depends on the reader recognizing the source anyway, the transposition didn't actually change anything load-bearing.

**When a major factual error is discovered late enough to require reconstructing a whole passage or thread** (not just correcting one claim): separate what's being rebuilt from what's staying fixed before starting. Keep every already-validated mechanism, object, and line of dialogue that doesn't depend on the wrong fact; rebuild only the factual layer itself (the geography, the date, the specific real-world detail) around what's kept. Treating the whole passage as needing to be rewritten from scratch loses validated work that the factual error never actually touched.
...
**Do:** treat divergence between two readers on the same point as a result, not noise — a split reaction is itself information (the text may be doing something genuinely ambivalent, on purpose or not) and deserves investigation before being resolved either way. When that divergence runs in opposite directions on the same specific passage (one reader wants it cut, another names it a highlight), treat it by default as evidence *against* a defect existing there rather than as an open question — a stronger, third independent reader is what would move that default, not further deliberation on the existing two responses. See the reader-convergence protocol in `protocols.md` for the full mechanics of turning multiple responses into a decision: the branch a signal falls into, why matching *valence* (not just matching location) is required for convergence to count, why a purely technical/factual claim inside a response doesn't need any convergence threshold at all, and where the reader count itself should stop being treated as more signal and start being treated as a distribution of taste.
...
**The stopping rule for getting to freeze in the first place, stated explicitly:** the point to declare freeze is not "no further improvement is conceivable" — it's the point where continued scene-by-scene optimization has a real cost that starts to outweigh its gain (a text over-smoothed into a controlled minimalism that no longer reads as a lived voice, diminishing returns on each further pass, the author's own judgment growing less reliable from over-exposure to the material). Naming that cost explicitly, in writing, is what turns "we could still improve this" from an open-ended reason to keep revising into a closed one — recognizing that the marginal revision now costs more than it returns is itself the freeze decision, not a postponement of it.

**If a separate, cleaned-up artifact is produced from the frozen manuscript at or after freeze** (a reading copy with working headers stripped, spacing normalized, a format-specific derivative): declare explicitly, at the point it's produced, that it derives from the frozen text by a purely mechanical transformation — list the transformations — and state which of the two files remains the source of truth for any future correction. Any further copy derived for another purpose (a different format, an anthology, a translation) must derive from that declared source of truth, never from an earlier working file — verify this filiation explicitly before syncing any such copy, rather than assuming it. A silent divergence here (a copy quietly derived from the wrong ancestor) can sit undetected for a long time, because both files look plausible on their own and nothing in the ordinary editing workflow would surface the mismatch.
