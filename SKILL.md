...
2. **Plot before demonstration.** A reader should be pulled forward by wanting to know what happens, never by an argument the book is making at them. This holds even — especially — for novels built on real research or a thesis-shaped premise.
...
The single most important discipline in this skill, because almost every other protocol depends on getting it right. Any claim made about a manuscript falls into exactly one of four categories, and the categories are not interchangeable:
...
The dangerous move is sliding from one category to the next without noticing: "three readers dropped off here" is a *reading effect* (data — three independent reports). "This scene is too long" is a *causal interpretation* (a theory about the cause). The first is close to unimpeachable; the second is one hypothesis among several (wrong POV, misplaced information, weak stakes, bad prose rhythm — any of these could produce the same drop-off) and must be tested before being acted on. Every audit and every procès in this skill exists partly to enforce this distinction. When reporting a finding — your own or a reader's — always name which category it is before proposing what to do about it.
