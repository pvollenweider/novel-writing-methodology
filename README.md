...
`evals/evals.json` holds the test prompts used to check the skill actually changes behavior versus Claude's unassisted judgment. Three scenarios (research→architecture, scene diagnosis, reader-feedback triage after a near-freeze) were run with and without the skill and graded against explicit assertions. Summary from the first iteration: the skill made the largest difference on the research→architecture case (Claude's unassisted answer stayed at the level of theme and character want, never reaching a reader-effect question or the scene-necessity test), a moderate difference on scene diagnosis, and no measurable difference on reader-feedback triage — Claude's default judgment on triaging beta-reader feedback was already close to what the skill prescribes, which says more about that eval than about the skill (a case study on where the skill's marginal value actually lives, not a uniform "always helps" story).
